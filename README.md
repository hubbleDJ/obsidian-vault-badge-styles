...
- Настраивать значения свойств глобально, если одно значение должно выглядеть одинаково в разных свойствах.
...
Если указать свойство, правило сработает только для этой пары:

```text
Status = done
```

Если оставить свойство пустым, правило станет глобальным для значения:

```text
done
```

Точное правило `свойство + значение` сильнее глобального правила по одному значению.

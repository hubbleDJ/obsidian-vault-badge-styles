...
- `icons/` - исходники встроенных иконок плагина.
- `README.md` - пользовательское описание на русском.
- `PLUGIN_CONTEXT.md` - этот технический файл.

В проекте нет сборки через `npm`/`esbuild`: публикуются напрямую `main.js`, `manifest.json`, `styles.css`. BRAT устанавливает только эти три файла, поэтому встроенные иконки из `icons/` дополнительно зашиваются в `main.js` как generated fallback. Плагин не создает файлы иконок при загрузке.
...
Встроенные иконки лежат обычными файлами в `icons/` внутри репозитория плагина и при ручной установке могут лежать в `.obsidian/plugins/vault-badge-styles/icons`. Для BRAT в `main.js` есть сгенерированная fallback-копия текущей папки `icons/`, потому что BRAT не скачивает папки из release. `IconResolver` рекурсивно сканирует пользовательские папки поиска и установленную папку встроенных иконок, строит индекс относительных путей и коротких имен, а пользовательские папки имеют приоритет выше встроенной и fallback-копии.
...
Перекраска SVG сделана через inline SVG и `currentColor`. Прозрачный фон SVG остается прозрачным, а простые SMIL-анимации через `animate`/`animateTransform` сохраняются. Перед вставкой SVG санитайзится: удаляются `script`, `style`, `foreignObject`, inline event handlers и внешние `href/src`. Для PNG/WebP/JPG/JPEG цвет иконки не меняется.
...
- рекурсивный индекс файлов иконок в подпапках;
- проверка расширений;
...
- SVG recolor работает через inline SVG и `currentColor`; `mask` остался только как fallback, если SVG не удалось распарсить.
...
Встроенные иконки входят в release через generated fallback в `main.js`; исходная папка `icons/` остается в git/tag для ручной установки и разработки. Если меняешь файлы в `icons/`, нужно пересгенерировать блок `BUNDLED_ICON_DATA` перед релизом.
